...
public abstract class BaseModel<C extends Constructible> implements ModelImpl {
    ...
    public C filter(OASFilter filter, Map<Object, Object> stack) {
        this.modCount += filter(filter, stack, properties);

        // TODO: check if instance of BaseModel<C> and convert if not.
        return filter(filter);
    }

    private static int filter(OASFilter filter, Map<Object, Object> stack, Map<String, Object> map) {
        ...
        while (cursor.hasNext()) {
            ...
            if (stack.containsKey(value)) {
                ModelsLogging.logger.cylicReferenceDetected();
            } else if (value instanceof BaseModel) {
     ...
     */
    public <T extends BaseModel<C>> T merge(T other) {
